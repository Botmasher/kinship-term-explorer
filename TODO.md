...
- [X] cite sources (now handled in frontend)
...
### data
- [ ] rework `NodesManager` to take in terms data from external app
	- [X] draft a `SetMemberLabel` function to relabel individual family members
	- [ ] account for marked terms (currently handled with M/F keys nested within types in the JSON)
	- [ ] try messaging this function externally
